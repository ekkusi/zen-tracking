...
export const loaderResetors = {
  ...
  clearParticipationsCache: async (participationId: string, loaders: DataLoaders) => {
    ...
    else throw new Error("Couldn't clear participationsLoaders, no participation found with id " + participationId);
  },
  // participationsLoader cache clearer when only access to user name
  clearParticipationsCacheByUser: async (name: string, loaders: DataLoaders) => {
  ...
  clearParticipationsCacheByChallenge: async (challengeId: string, loaders: DataLoaders) => {
    const participations = await prisma.challengeParticipation.findMany({
      where: { challenge_id: challengeId }
    });
    participations.forEach(it => {
      console.log("Clearing caches for participation: " + it.id + " user: " + it.user_name);
      loaders.challengeParticipationsLoader.clear(it.challenge_id);
      loaders.userParticipationsLoader.clear(it.user_name);
    });
  },
  ...
  clearMarkingsCache: async (markingId: string, loaders: DataLoaders) => {
    ...
    else throw new Error("Couldn't clear markingsLoader, no marking found with id " + markingId);
  }
};
...
const dataLoaders: DataLoaders =  {
    ...
    userLoader: new DataLoader(async (keys) => {
      ...
      console.log(
        `userLoader loaded users: ${JSON.stringify(users)}`
      );
      return users;
    }),
    challengeParticipationsLoader: new DataLoader(async (keys) => {
      ...
      console.log(
        `challengeParticipationsLoader loaded participations: ${JSON.stringify(participations)}`
      );
    ...
    userParticipationsLoader: new DataLoader(async (keys) => {
      ...
      console.log(
        `userParticipationsLoader loaded participations: ${JSON.stringify(participations)}`
      );
